...
public class MusicRepository {
    ...
    private Connection getConnection() throws SQLException {
        return DriverManager.getConnection("jdbc:postgresql://localhost:5432/OOAP2.4", "postgres", "Kto prochital tot zdohnet");
    }
    ...
    public List<Album> getAlbumsByArtistId(int artistId) {
        ...
        String sql = "SELECT a.id, a.title, a.cover_path FROM albums a " +
                "JOIN artist_albums aa ON a.id = aa.album_id " +
                "WHERE aa.artist_id = ? ORDER BY a.id";
    ...
    public Track getTrackById(int trackId, List<Track> tracksInMemory) {
        for (Track track : tracksInMemory) {
            if (track.getId() == trackId) {
                return track;
            }
        }

        String sql = "SELECT t.id, t.title, t.is_added, a.cover_path FROM tracks t " +
        ...
        try (Connection conn = getConnection();
            ...
            try (ResultSet rs = pstmt.executeQuery()) {
                if (rs.next()) {
                    Track track = new Track(rs.getInt("id"), rs.getString("title"), rs.getBoolean("is_added"));
    ...
    public List<Track> getTracksByAlbumId(int albumId, List<Track> tracksInMemory) {
        ...
        try (Connection conn = getConnection();
            ...
            try (ResultSet rs = pstmt.executeQuery()) {
                while (rs.next()) {
                    ...
                    Track track = getTrackById(trackId, tracksInMemory);
                    if (track != null) {
                        tracks.add(track);
                    }
                }
